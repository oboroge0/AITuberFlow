...
- **エクスポート時にファイル保存ダイアログを表示** - デスクトップ版でワークフローをエクスポートすると保存先を選べずダウンロードフォルダ等へ確認なしで直接保存されていた問題を修正。保存先を選択できるようになった (#296)

## [2.6.1] - 2026-07-03

### 修正

- **オーバーレイのWebSocket自動再接続** - 接続が瞬断した際にOBS上の画面が復帰せず固まったままになる問題を修正。自動で再接続するようにした (#290)
- **サーバーのバインド先を既定でlocalhostに限定** - セキュリティ強化のため、既定では `127.0.0.1` のみで待ち受けるように変更。**挙動変更**: 別マシンのOBS等からサーバーを参照している場合は、環境変数 `HOST=0.0.0.0` を設定してLAN公開を明示的に有効にする必要がある (#291)
- **LLMノード失敗時の誤読み上げを防止** - LLM呼び出しが失敗した際、エラー文字列がそのまま正常な応答として配信・読み上げられてしまう事故を修正 (#292)
- **プラグインロード経路の検証強化** - パス検証を追加し、読み込み失敗時にはエラーを明示的に通知するように修正 (#293)
- **APIキー等の機微情報の露出を修正** - APIレスポンスにAPIキー等が平文で含まれていた問題を修正。GET取得やエクスポート時はマスク値 `********` を返すようにした（保存・実行時の動作には影響なし） (#294)

## [2.6.0] - 2026-07-01

### 追加

- **ライト/ダークモード** - アプリ全体にライト/ダークテーマを導入。ヘッダーの月/太陽アイコンで切り替え、選択は保存され、起動時のちらつきも防止。ホーム・エディタ（ノード本体・接続線を含む）・各パネル・設定モーダル・お知らせ帯まで対応。ダークは従来デザインを維持し、接続線はテーマに応じて自動でコントラストを確保する (#282)

### 修正

- **デスクトップ版で主要機能が動かない問題を修正** - フロントを `http://localhost:<port>` で表示している都合で Tauri IPC がページに届かず、**自動アップデート・ワークフローのエクスポート保存・オーバーレイの別ウィンドウ表示**が動作していなかった。capability に `remote.urls` を追加して localhost 配信ページに IPC を許可し、実機で動作するようにした (#280)
- **暗いノード色がダーク背景に同化する問題を修正** - Ollama / OBS のアクセント色を視認性の高い色（スチールブルー / ペリウィンクル）に変更し、ノードのアイコンチップと**接続線**が見えるようにした。暗い色は自動で明度を持ち上げるガードも追加 (#281)

## [2.5.2] - 2026-06-29

### 追加

- **ホーム画面にDiscordリンク** - フッターにDiscordサーバーへの参加リンクを追加
- **デスクトップ更新通知をフッターの更新ボタンに変更** - モーダルダイアログからフッターの控えめなボタンに変更 (#277)

### 修正

- **アバターデフォルトモデル** - 存在しない avatar.vrm から Flowchan.vrm に変更
- **VRMロードエラー** - モデルファイルが存在しない場合に明確なエラーメッセージを表示（404/HTMLレスポンスの事前検知）
- **VRMプレビューの見切れ** - 詳細設定パネル開閉時にResizeObserverでレンダラーサイズを自動更新
- **console-outputノード** - textを出力ポートから返すよう修正（manifest.jsonにoutput定義追加）
- **i18nキー欠落** - inlineセクションの16キーを追加、重複キー3個を削除

## [2.5.1] - 2026-06-28

### 追加

- **ポート説明文** - 全37プラグイン（118ポート）にdescriptionを追加。ポートホバー時に「読み上げるテキスト」「生成された音声ファイル」等の1行説明を表示 (#269)
- **エディタのローディングオーバーレイ** - ワークフロー読み込み中にスピナーを表示 (#273)
- **Ollamaノード強化** - プロンプトビルダー・キャラクター人格設定・maxTokens対応 (#270)

### 変更

- **ノード表示モードを標準のみに統一** - 簡易/詳細モードを廃止（264行削除） (#271)
- **i18n完全対応** - ハードコード文字列86個をロケールファイルに移行。日英94キー追加 (#271)

### 修正

- **出力→出力の接続を禁止** - ConnectionMode.Strictに変更 (#272)
- **ノード背景を不透明に変更** - 重なった下のノードが透けて操作を奪う問題を解消 (#272)
- **ドラッグ時のノード最前面表示** - elevateNodesOnSelect + onNodeDragStart + elevateEdgesOnSelect (#272)
- **ドラッグ中のポートハイライトを反対側のみに制限** - 出力→出力のハイライトを抑止 (#272)
- **Tooltipをポータル描画** - 他のノードに隠れず常に最前面表示 (#272)
- **PortTooltipの縦文字修正** - whitespace-nowrap追加 (#272)
- **audioポートの型修正** - TTS系5プラグイン+audio-playerのaudioポートがstring型だったのをaudio型に修正 (#272)
- **ポートdescriptionのパススルー修正** - Canvas→CustomNodeの変換でdescriptionが落ちていた問題を修正 (#272)
- **VRM非表示時のCPU消費** - Page Visibility APIで非表示時にrequestAnimationFrameを停止 (#273)
- **エディタ読み込み速度改善** - API並列化、プラグイン並列読み込み+メモリキャッシュ (#273)
- **サーバー親プロセス監視** - ポーリング間隔を1秒→5秒に延長 (#273)

## [2.5.0] - 2026-06-27

### 追加

- **アクティビティドロワー** - ログ表示を刷新。実行サイクル（トリガー発火1回分）単位で「時刻・トリガー内容・応答冒頭・所要時間」を一覧表示し、▽で展開すると実行順のステップリスト（ノード名・所要時間・結果）を表示。ステップクリックで該当ノードへジャンプ。従来のコンソール形式は「生ログ」タブに残置 (#194, #236)
- **ノード設定の完全インライン化（ComfyUI式）** - 全設定フィールドをノード内で直接編集可能に。フィールドごとの折りたたみ、password/textarea/スライダー/トグル対応。旧右パネルは廃止し、複雑なフィールド用に「ノード詳細設定」パネルを新設 (#193, #202, #204)
- **「グローバル設定を使用中」バッジ** - ノード側が未設定でグローバル設定の値が使われるフィールドに「グローバル」バッジを表示。要約値も実際に使われるグローバル値を表示 (#233, #235)
- **ノード実行ステータスのビジュアル表示** - ノードのボーダーカラーが実行状態に連動（青=実行中/緑=成功/赤=エラー/黄=警告）。ホバーで実行詳細（所要時間・結果・エラー）をポップオーバー表示、エッジの色も連動 (#201)
- **ワークフロー接続の整合性チェック** - 不正な接続（存在しないポート等）を実行前に検出して警告 (#188)
- **ポート接続UXの改善** - ドラッグ中の接続候補ハイライト、ポート型バリデーション、互換ノード候補表示 (#176, #187)
- **TTS系プラグインのaudioUrl出力ポート** - VOICEVOX/COEIROINK/AivisSpeech/OpenAI TTS/Style-Bert-VITS2のmanifestにaudioUrl出力を追加 (#205, #220)
- **ローディングインジケーター** - ホームページの長時間処理に追加 (#190, #196)
- **右クリックメニューのカテゴリ別サブメニュー化**

### 変更

- **プラグイン設定キーをcamelCaseに統一** - 旧snake_caseのワークフローは読み込み時に自動解決（後方互換あり、再保存で新形式に移行） (#104, #189, #192)
- プラグイン間のイベント発火パターンとTTS返却オブジェクト形状を統一 (#121)
- フロントエンドのエラーハンドリングを統一、alert/confirmをトースト通知に置換 (#103, #200)
- GLOBAL_SETTINGS_MAPを共通モジュールへ一元化 (#209, #223)
- CI・テスト・lintの実行範囲を整備（dev向けPRでもCIが実行されるように、テスト範囲をtests/全体に拡大） (#206, #221)
- ESLint設定をflat configに一本化、警告を全件解消 (#199, #217, #230)

### 修正

- **ワークフロー停止のハング** - Twitch接続が確立できない状態で停止が完了しない問題を修正。プラグインのsetup/teardownにタイムアウトを導入し、どのプラグインがハングしても停止が有界時間で完了 (#239, #240)
- **連続TTSの音声キュー** - 発話中に次の音声が来ても中断せず順番に再生。実行の再開始・停止時はキューを破棄 (#218, #226, #232)
- **ノードドラッグの性能** - ドラッグ中の全ノード再レンダリングを解消しカクつきを修正 (#232)
- **インラインフィールドの初期表示** - プラグイン定義ロード中はスケルトンを表示（後から突然出現しない） (#232)
- **VTube Studio再接続の堅牢性** - 指数バックオフ（3秒→最大60秒）、接続タイムアウト、リトライループの修正 (#207, #222)
- **デスクトップ起動エラーの可視化** - サイドカー起動失敗時に無言でクラッシュせずエラーダイアログを表示。ポート確保のレース対策も追加 (#208, #224)
- ワークフローロックのレースコンディションを修正 (#149, #197)
- オーバーレイとVRMレンダラの安定性バグを修正 (#151, #198)
- TTS/リップシンクプラグインがエラーを飲み込む問題を修正 (#201)
- グローバル設定の保存後にダイアログを自動で閉じる (#191, #195)
- グレースフルシャットダウンの整備 - 終了時に実行中ワークフローを停止しWALチェックポイント後にDBクローズ。シャットダウンテストを追加 (#149, #237)
- /validateエンドポイントが壊れたJSONに400を返すように修正 (#237)
- devのフロントエンドビルドを壊していた型エラーを修正 (#227, #228)
- 依存関係の脆弱性対応（tar 0.4.46、npm audit対応） (#169, #175)

### ドキュメント

- ブランチ運用とリリースフロー（dev/main二段構え）をCLAUDE.mdに明文化 (#231)
- 日英ドキュメントの内容差異を同期、READMEにダウンロード統計チャートを追加 (#171, #215, #225)
- リポジトリ衛生: .serena/AGENTS.md/.playwright-mcp をgitignoreへ (#219, #229)
...
- ワークフローバリデーションがグローバル設定のAPIキーを考慮せず、設定済みでもエラーになる問題を修正 (#146)
