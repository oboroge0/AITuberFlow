...
```bash
# プロジェクトルートから全サブプロジェクトの依存関係をインストール
npm run setup
```
...
バックエンドは `http://localhost:8001` で起動します。デフォルトでは `127.0.0.1` にのみバインドされ、同一LAN上の他のマシンからはアクセスできません。

OBSやブラウザオーバーレイを別マシンで動かす場合は、`HOST=0.0.0.0` を設定するとLAN全体からアクセスできる従来の挙動に戻せます：
